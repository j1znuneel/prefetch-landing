...
export const PredictiveMock = () => {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      if (!containerRef.current || !buttonRef.current || !logoRef.current) return;
      ...
      if (x >= 0 && x <= rect.width && y >= 0 && y <= rect.height) {
        ...
        const getMinDist = (targetRect: DOMRect) => {
          const b = { left: targetRect.left - rect.left, top: targetRect.top - rect.top, right: targetRect.right - rect.left, bottom: targetRect.bottom - rect.top };
          const dx = Math.max(b.left - x, 0, x - b.right);
          const dy = Math.max(b.top - y, 0, y - b.bottom);
          return Math.sqrt(dx * dx + dy * dy);
        };

        const distance = Math.min(getMinDist(buttonRect), getMinDist(logoRect));
        const maxDist = 300;
  ...
  return (
    <div className="relative w-full max-w-5xl px-4 py-12">
      ...
      <div className="bg-white rounded-[2rem] border border-slate-100 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.08)] overflow-hidden">
        ...
        <div className="browser-header">
          ...
          <div className="dot" />
          <div className="dot" />
          <div className="ml-4 px-3 py-1 bg-slate-50 rounded-md text-[10px] font-bold text-slate-400 uppercase tracking-widest">
        ...
        >
          {/* Probability Indicator (Simplified Napkin style) */}
          <div className="absolute top-12 left-12 flex flex-col items-start">
             ...
             <div className="text-4xl font-extrabold text-black tabular-nums leading-none">
          ...
          <div className="relative z-10 flex flex-col items-center gap-6">
            ...
            <div 
              ref={logoRef}
              className={cn(
                "w-14 h-14 bg-black text-white rounded-2xl flex items-center justify-center font-black text-2xl transition-colors duration-500",
            ...
            <button
              ref={buttonRef}
              className={cn(
                "relative px-12 py-5 bg-black text-white rounded-2xl font-extrabold text-xl transition-all duration-500 shadow-xl",
            ...
            <div className={cn(
              ...
              status === "idle" ? "text-slate-200" : "text-slate-400"
            )}>
              {status === "idle" && "Move cursor to test"}
